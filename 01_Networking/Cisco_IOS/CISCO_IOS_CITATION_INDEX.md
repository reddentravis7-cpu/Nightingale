...
| Command / field family | Section or anchor | Confirmed by | Date |
|---|---|---|---|
| router ospf | `ospf-i1.html` ("ip ospf fast-reroute per-prefix through R") | Purpose + real usage example confirmed; formal syntax table (process-id range) not reached — page truncated | 2026-07-27 |
...
| show clock, show users | attempted, not found | Two and three real navigation attempts respectively, command not reachable in the pages tried — see "Still uncited" below | 2026-07-27 |
...
- **show-clock** — real content exists (WebSearch snippet quoting matching cisco.com text: "show clock command in User Exec mode or Privileged Exec mode...no arguments or keywords"), but not pinned to an exact page. Tried: `cf_s1.html`, `cf_s2.html` (searched within already-fetched text, not found). Not yet tried: the E-Learning archival family (no `show_clock.htm`-style URL was surfaced in search results, unlike show_ip_route/hostname/etc.) — worth a direct search for that specific archival URL before trying more chapter books.
- **show-users** — real content exists (WebSearch snippet referencing the Cisco IOS Configuration Fundamentals Command Reference), not pinned. Tried: `cf_s4.html`, `cf_s5.html` (fetched directly, command not found in reachable text).
- **router-ospf process-id range** — command and purpose confirmed real on `ospf-i1.html`; the specific "1-65535" numeric range was not independently re-confirmed from source text because the page truncated before reaching the formal syntax table. The block is otherwise solid; this is the one specific fact still open.
