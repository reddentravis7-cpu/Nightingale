# Translate Scope Constitution v2.0

**Document ID:** TRANSLATE-CONST-v2.0
...
**Change from v1.0:** the common-obligations section has been **extracted
into a standalone base document, Common Constitutional Obligations (CCO
v1.0)**, which this Constitution now references rather than restates. The
scope/harm-boundary content below is **unchanged from v1.0** — citations to
TRANSLATE-CONST for the harm boundary remain valid in substance. See the
...
## Common obligations of every governance role

*Extracted to a standalone base document in v2.0.* Beyond the scope/harm
boundary above, every governance role is bound by a shared floor of
conduct while doing its work. As of v2.0 those obligations no longer live
in this document — they are defined in the **Common Constitutional
Obligations (CCO v1.0)**, a standalone base document subordinate to this
Constitution.

CCO is the **single authoritative home** for the eight obligations, the
roster of roles bound by them, and the versioning rule by which each role
document cites the CCO version it implements. This Constitution
deliberately does **not** reproduce any of them here — keeping exactly one
authoritative copy is the entire point of the extraction. Individual role
documents define only what is specific to that role; they do not restate
the obligations — the same relationship a Knowledge Block's typed content
has to its shared envelope in `KNOWLEDGE_BLOCK_MODEL.md`.

CCO is subordinate to this Constitution and overrides no clause of it;
where the two conflict, this Constitution governs. The extraction does not
resolve the open IP/copyright question flagged at the top of this document
— that flag is authoritative here, and CCO Obligation 4's relationship to
it is stated in CCO itself.
...
## Amendment — v1.0 → v2.0 (2026-07-29)

**What changed.** The "Common obligations of every governance role"
section that v1.0 carried in full (the eight obligations, the role
roster, and the base/derived rule) was **extracted into a standalone base
document, Common Constitutional Obligations (CCO v1.0)**. This section now
references CCO rather than restating it.

**What is new, and must be reviewed as new — not waved through as "moved."**
CCO does not merely relocate v1.0's bare eight-item list. It elaborates
each obligation, and adds machinery that did not exist in v1.0: a
versioning-and-citation regime (CCO §4) and a rule that Obligation 6
(auditable record) is discharged through the Database Manager's substrate
(CCO §5). These are reasonable and consistent with existing precedent, but
they are **new normative content** — CCO §3–§5 should be ratified as new
text, not assumed identical to the v1.0 obligations paragraph. Only the
harm-boundary body carries the "substantively identical" guarantee below.

**Why.** To give the shared obligations their own version line,
independent of this Constitution's other clauses, so they can evolve and
be re-ratified against each role deliberately without touching the
harm-boundary text — and so there is exactly one authoritative copy of the
obligations rather than a copy embedded here and a copy elsewhere. (Architect
decision, 2026-07-29.)

**What did not change.** Everything above the common-obligations section —
the entire scope/harm-boundary constitution — is **substantively identical
to v1.0**. No word of the harm boundary, the sentence-level enforcement
